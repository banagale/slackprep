...
def load_bot_users(users_path: Path) -> set:
    ...
    with open(users_path) as f:
        users = json.load(f)
        return {u["id"] for u in users if u.get("is_bot", False)}
...
def reassemble_messages(
    ...
    print(
        f"📊 Processed {total_convos} conversations: "
        f"{stats['channels']} channels, {stats['dms']} DMs, "
        f"{stats['group_msgs']} group messages"
    )

    if any(stats[k] for k in ["filtered_channels", "filtered_bot_msgs", "filtered_content_msgs"]):
        print(
            f"🔽 Filtered out: {stats['filtered_channels']} automation channels, "
...
def write_markdown(lines: list[str], output_path: Path, toc_entries=None, stats=None):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        if stats:
            f.write(
                f"**Export Summary**: {stats['channels']} channels, "
                f"{stats['dms']} DMs, {stats['group_msgs']} group messages\n"
            )
            if any(stats[k] for k in ["filtered_channels", "filtered_bot_msgs", "filtered_content_msgs"]):
                f.write(
                    f"**Filtered**: {stats['filtered_channels']} automation channels, "
                    f"{stats['filtered_bot_msgs']} bot messages, "
                    f"{stats['filtered_content_msgs']} automated content\n"
                )
